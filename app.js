...
function setCat(c){cat=c;render();renderFilters()}function renderFilters(){$("filters").innerHTML=["Wörter","Zahlen","Alltag"].map(c=>`<button onclick="setCat('${c}')">${c}</button>`).join("")}
...
function guess(id){if(id===question.id){stars++;localStorage.setItem("kkstars",stars);$("stars").textContent=stars;$("gmsg").textContent="⭐ 好嘢!";setTimeout(newGame,700)}else{$("gmsg").textContent="再聽一次 👂";play(question)}}
...
renderFilters();render();if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js");
